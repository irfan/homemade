...
<section id="codes">
	...
	<p><?= $projectsHeader['custom']['_aioseop_description'][0]; ?></p>
	</header>
	<? foreach($projectPages as $page): ?>
		
		<?php $data = getProjectInfo($page); ?>
        <article class="column">
			<h2><a href="<?= $data['permalink'] ?>" title="<?= $data['desc']['_aioseop_description'][0]; ?>"><?= apply_filters('the_title', $data['title']); ?></a></h2>
			<p class="headerTitle"><?= $data['desc']['_aioseop_description'][0]; ?></p>
		</article>
	<? endforeach; ?>
</section>
